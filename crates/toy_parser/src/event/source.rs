...
pub struct Source<'a> {
    ...
    pending_errors: Vec<toy_lexer::LexError>,
}
...
impl<'a> Source<'a> {
    ...
    pub(crate) fn take_pending_errors(&mut self) -> Vec<toy_lexer::LexError> {
    ...
    pub(crate) fn has_newline_before_next(&mut self) -> bool {
        ...
        self.buffer
            .iter()
            .any(|t| t.kind == toy_lexer::TokenKind::NewLine)
    }
