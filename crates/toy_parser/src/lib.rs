...
pub fn parse(input: &str) -> (ResolvedNode, Vec<ParseError>) {
    ...
    let sink = Sink::new();
    let (green_node, interner, mut parse_errors) = sink.build(events);
    ...
    (
        ResolvedNode::new_root_with_resolver(green_node, interner),
        parse_errors,
    )
}
