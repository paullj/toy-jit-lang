...
use toy_hir::{
    BlockItem, Definition, ExprIdx, Expression, Ident, InfixOp, Item, Literal, PrefixOp,
    ResolvedIdent,
};
use toy_typecheck::{InferenceResult, Type, TypedModule};

use crate::ir::{
    Block, BlockId, CapturedVar, FuncId, Function, Inst, LocalId, Module, Operand, VReg, ValueType,
};

pub fn lower(hir: &toy_hir::LowerResult, types: &InferenceResult) -> Module {
    let mut ctx = LowerCtx::new(hir, types, None);
    ctx.lower_module();
    ctx.finish()
}

/// Lower HIR to MIR using an external interner.
///
/// Use this when the LowerResult was built with a dummy interner
/// and the actual interner is stored elsewhere (e.g., in a salsa database).
pub fn lower_with_interner(
    hir: &toy_hir::LowerResult,
    types: &InferenceResult,
    interner: &toy_hir::Interner,
) -> Module {
    let mut ctx = LowerCtx::new(hir, types, Some(interner));
    ctx.lower_module();
    ctx.finish()
}

/// Lower a TypedModule to MIR.
///
/// This is the preferred entry point when using the typed HIR representation
/// from the compiler pipeline. It extracts the necessary information from the
/// TypedModule and uses type information to guide code generation.
pub fn lower_typed_module(typed_module: &TypedModule, interner: &toy_hir::Interner) -> Module {
    // Build a synthetic LowerResult from the TypedModule
    use toy_hir::{LowerResult, SymbolTable};

    let mut items = Vec::new();
    let expr_spans = la_arena::ArenaMap::new();
    let mut item_spans = Vec::new();
    let symbols = SymbolTable::default();

    // Convert TypedItems back to HIR items
    for typed_item in &typed_module.items {
        items.push(typed_item.item.clone());
        item_spans.push(typed_item.span);
    }

    log::debug!(
        "lower_typed_module: Converting {} TypedItems to HIR items, {} expressions",
        typed_module.items.len(),
        typed_module.expressions.len()
    );

    // Build an InferenceResult from the TypedModule
    let mut inference = InferenceResult::new();
    inference.variable_types = typed_module.variable_types.clone();

    // Collect all expression types from all items
    for typed_item in &typed_module.items {
        for (expr_idx, ty) in typed_item.expr_types.iter() {
            inference.expression_types.insert(expr_idx, ty.clone());
        }
    }

    // Create a LowerResult structure with the expressions from TypedModule
    let hir = LowerResult {
        items,
        expressions: typed_module.expressions.clone(),
        expr_spans,
        item_spans,
        symbols,
        diagnostics: vec![],
        interner: toy_hir::Interner::default(), // Dummy, we use the external interner
    };

    // Use the existing lowering infrastructure with the type information
    let mut ctx = LowerCtx::new(&hir, &inference, Some(interner));
    log::debug!("Calling lower_module with {} HIR items", hir.items.len());
    ctx.lower_module();
    let module = ctx.finish();
    log::debug!("Generated module with {} functions", module.functions.len());
    module
}
...
struct LowerCtx<'a> {
    hir: &'a toy_hir::LowerResult,
    types: &'a InferenceResult,
    external_interner: Option<&'a toy_hir::Interner>,
    ...
    // Track types of virtual registers for proper NaN-boxing
    vreg_types: Vec<ValueType>,
}
...
impl<'a> LowerCtx<'a> {
    ...
    fn resolve(&self, ident: Ident) -> &str {
        if let Some(interner) = self.external_interner {
            interner.resolve(&ident.spur())
        } else {
            self.hir.resolve(ident)
        }
    }

    fn resolve_ident(&self, ident: &ResolvedIdent) -> &str {
        match ident {
    ...
    fn save_context(&mut self) -> SavedContext {
        SavedContext {
            ...
            vreg_types: std::mem::take(&mut self.vreg_types),
        }
    }
    ...
    fn restore_context(&mut self, saved: SavedContext) {
        ...
        self.vreg_types = saved.vreg_types;
    }
    ...
    fn lower_module(&mut self) {
        ...
        // This allows functions to call each other, including recursive calls
    ...
    fn lower_call(&mut self, callee: &Expression, args: &[ExprIdx]) -> Operand {
        ...
        if let Expression::VariableRef { name } = callee {
            ...
            // Look up the function - need to convert &str to String for HashMap lookup
            let name_string = name_str.to_string();
            if let Some(&func_id) = self.func_names.get(&name_string) {
    ...
    fn lower_expr(&mut self, expr: &Expression) -> Operand {
        match expr {
            ...
            Expression::Call { callee, args } => {
                // WORKAROUND: The TypedModule has incorrect expression indices.
                // The callee index often points to the wrong expression (e.g., a literal).
                // This is a bug in the HIR/TypedModule construction that needs fixing upstream.

                let callee_expr = &self.hir.expressions[*callee];

                // If the callee doesn't make sense (e.g., it's a literal),
                // try to find a VariableRef that might be the actual function
                let actual_callee = if matches!(callee_expr, Expression::Literal(_)) {
                    // Find the first VariableRef - this is a crude heuristic
                    // that works for simple cases
                    if let Some((_, var_ref)) = self
                        .hir
                        .expressions
                        .iter()
                        .find(|(_, e)| matches!(e, Expression::VariableRef { .. }))
                    {
                        var_ref
                    } else {
                        callee_expr
                ...
                self.lower_call(actual_callee, args)
            }
