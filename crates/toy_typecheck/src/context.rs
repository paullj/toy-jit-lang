...
use toy_hir::{
    BlockItem, Definition, ExprIdx, Expression, Ident, Item, Literal, ResolvedIdent, TextRange,
};
...
pub(crate) struct InferCtx<'a> {
    ...
    /// External interner override (for incremental compilation)
...
impl<'a> InferCtx<'a> {
    pub(crate) fn new(hir: &'a toy_hir::LowerResult) -> Self {
        Self {
            hir,
    ...
    pub(crate) fn with_interner(
        hir: &'a toy_hir::LowerResult,
        interner: &'a toy_hir::Interner,
    ) -> Self {
        Self {
            hir,
            external_interner: Some(interner),
            env: TypeEnv::new(),
            subst: Subst::new(),
            next_var: 0,
            expr_types: ArenaMap::default(),
    ...
    pub(crate) fn with_env(hir: &'a toy_hir::LowerResult, env: TypeEnv, next_var: u32) -> Self {
        Self {
            hir,
            external_interner: None,
            env,
    ...
    /// Resolve a ResolvedIdent to its string representation
    ...
    fn infer_infix(&mut self, op: toy_hir::InfixOp, lhs: ExprIdx, rhs: ExprIdx) -> Type {
    ...
    fn infer_prefix(&mut self, op: toy_hir::PrefixOp, expr: ExprIdx) -> Type {
