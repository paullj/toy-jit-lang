...
List type annotations use the syntax `[Type]`, where `Type` is the type of the elements in the list. In the example above, `my_list` is a list of integers. The type annotation is optional if the type can be inferred from the context:

```toy
my_list = [1, 2, 3, 4, 5]  # inferred as [int]
```
...
You can destructure a list into individual variables:
